...
1. Dylan describes the change in this Claude Code chat
2. Claude edits local files at `/Users/dylanteo/Desktop/badmintoncourtfinder/`
3. Dylan opens the **Codespace** (`expert-orbit-pwp4pgqpgq3rx7x.github.dev`) in VS Code via the GitHub Codespaces extension (connected via Remote SSH)
4. Dylan runs a Python script in the **Codespace terminal** to apply the same changes to the Codespace files
5. From the **Codespace terminal**, Dylan runs `git push` and `fly deploy` (the Codespace is not subject to corporate restrictions)
...
| `fly.toml` | Fly.io app config (app name: `badmintoncourtfinder`, region: `sin`) |
...
- **Venue → Address mapping:** Need to build a lookup table in `config.yml` or a new file mapping venue keywords (e.g. "teck ghee") to full Singapore addresses
